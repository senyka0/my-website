...
const Home = ({ photoUrl, name, surname, position, location, theme }: { photoUrl: string | undefined; name: string | undefined; surname: string | undefined; position: string | undefined; location: string | undefined; theme: "light" | "dark" }) => {
  return (
    <div id="home" className={theme === "light" ? "p-10 pt-36 md:flex md:p-20 w-screen h-screen  md:pt-40 bg-gradient-to-r from-[#3378ff] to-[#9442fe]" : "p-10 md:flex md:p-20 w-screen h-screen  md:pt-40 bg-slate-700"}>
      ...
      <div className="flex flex-col md:ml-10 mt-2 space-y-10">
        ...
        <h2 className="text-xl md:text-2xl lg:text-5xl font-bold text-[#8892b0]">{position}</h2>
        <div className="flex ">
          ...
          <p className="text-xl md:text-4xl font-mono text-[#8892b0] ">{location}</p>
        </div>
      </div>
    </div>
  );
