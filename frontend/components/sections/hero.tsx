...
export function Hero({ cvData }: HeroProps) {
  ...
  const fallbackName = 'Arsenii Koniachenko';
  const fullName = cvData ? `${cvData.name} ${cvData.surname}` : fallbackName;
      ...
      <div className="mx-auto max-w-4xl text-center">
        ...
        >
          {cvData ? (
            ...
            <>
              Arsenii <span className="text-gradient">Koniachenko</span>
            </>
          )}
        </motion.h1>
        <motion.p
        ...
        >
          {cvData?.position || 'Full-Stack Blockchain Developer'}
        </motion.p>
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <span>{cvData?.location || 'Location'}</span>
        </motion.div>
        <motion.p
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          I build production-ready Web3 products with clean UX, robust backend systems, and
          practical smart contract integrations.
        </motion.p>
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
