...
const SKILL_CATEGORIES: Record<string, string[]> = {
  Frontend: ['React', 'Next.js', 'TypeScript', 'JavaScript', 'HTML', 'CSS', 'TailwindCSS', 'Vue', 'Angular', 'Redux'],
  Backend: ['Node.js', 'Express', 'Python', 'Django', 'FastAPI', 'Go', 'Rust', 'PostgreSQL', 'MongoDB', 'Redis'],
  Blockchain: ['Solidity', 'Ethers.js', 'Web3.js', 'Hardhat', 'Foundry', 'Smart Contracts', 'EVM', 'IPFS', 'DeFi'],
  Tools: ['Git', 'Docker', 'AWS', 'Vercel', 'CI/CD', 'Linux', 'Figma', 'Testing'],
};
...
function categorizeSkill(skill: string): string {
  ...
  for (const [category, skills] of Object.entries(SKILL_CATEGORIES)) {
    if (skills.some((s) => lowerSkill.includes(s.toLowerCase()) || s.toLowerCase().includes(lowerSkill))) {
      return category;
    }
  }
  return 'Other';
...
export function Skills({ cvData }: SkillsProps) {
  ...
  const orderedCategories = ['Frontend', 'Backend', 'Blockchain', 'Tools', 'Other'].filter(
    (cat) => groupedSkills[cat]?.length
  );
